...
std::vector<cv::Mat> load_topomap(const fs::path & directory, int width, int height)
{
  ...
  for (size_t index = 0; index < files.size(); ++index) {
    ...
    if (image.empty() || image.cols != width || image.rows != height) {
      throw std::runtime_error("topomap image size differs from ONNX input: " + files[index].second.string());
    }
...
public:
  ...
  {
    const std::string model_path = declare_parameter<std::string>("model_path", "model/vint.onnx");
    const std::string topomap_directory = declare_parameter<std::string>("topomap_directory", "");
    const int inference_threads = declare_parameter<int>("inference_threads", 2);
    radius_ = declare_parameter<int>("radius", 2);
    close_threshold_ = declare_parameter<double>("close_threshold", 3.0);
    waypoint_index_ = declare_parameter<int>("waypoint_index", 2);
    const int goal_node_parameter = declare_parameter<int>("goal_node", -1);
    const double context_rate = declare_parameter<double>("context_frames_per_second", 4.0);
    const double inference_rate = declare_parameter<double>("inference_rate_hz", 2.0);
    image_timeout_seconds_ = declare_parameter<double>("image_timeout_seconds", 2.0);
    max_v_ = declare_parameter<double>("max_v", 0.2);
    model_frame_rate_ = declare_parameter<double>("model_frame_rate", 4.0);
    if (topomap_directory.empty() || inference_threads < 0 || radius_ < 0 ||
      context_rate <= 0 || inference_rate <= 0 || image_timeout_seconds_ <= 0 ||
      max_v_ <= 0 || model_frame_rate_ <= 0)
    {
      throw std::runtime_error("invalid navigator parameters");
    }
    context_period_seconds_ = 1.0 / context_rate;
    ...
    goal_node_ = goal_node_parameter < 0 ? static_cast<int>(topomap_.size()) - 1 : goal_node_parameter;
    if (goal_node_ >= static_cast<int>(topomap_.size()) || goal_node_ < 0) {
      throw std::runtime_error("goal_node is outside the topomap");
    }
...
private:
  void on_image(const sensor_msgs::msg::Image::ConstSharedPtr message)
  {
    ...
    if (stamp < last_image_seconds_) {context_.clear();}
    if (stamp >= last_image_seconds_ && stamp - last_image_seconds_ < context_period_seconds_) {
      return;
    }
    try {
      ...
      RCLCPP_ERROR_THROTTLE(get_logger(), *get_clock(), 5000, "camera conversion failed: %s", error.what());
    }
  }

  geometry_msgs::msg::PoseStamped waypoint_pose(
    const VintPrediction & prediction, size_t goal_offset, int step, const rclcpp::Time & stamp) const
  {
    const size_t offset = (goal_offset * model_->waypoint_count() + step) * 4;
    const double x = prediction.waypoints[offset] * max_v_ / model_frame_rate_;
    const double y = prediction.waypoints[offset + 1] * max_v_ / model_frame_rate_;
    const double yaw = std::atan2(prediction.waypoints[offset + 3], prediction.waypoints[offset + 2]);
  ...
  void infer()
  {
    ...
    if (reached_goal_ || context_.size() != static_cast<size_t>(model_->context_length())) {return;}
    if (now().seconds() - last_image_seconds_ > image_timeout_seconds_) {return;}

    const int start = std::max(0, closest_node_ - radius_);
    const int end = std::min(goal_node_, closest_node_ + radius_);
    std::vector<cv::Mat> goals(topomap_.begin() + start, topomap_.begin() + end + 1);
    std::vector<cv::Mat> observations(context_.begin(), context_.end());
    ...
    const size_t closest_offset = static_cast<size_t>(std::distance(
      ...
      std::min_element(prediction.temporal_distances.begin(), prediction.temporal_distances.end())));
    const float minimum_distance = prediction.temporal_distances[closest_offset];
    const size_t subgoal_offset = std::min(
      closest_offset + static_cast<size_t>(minimum_distance < close_threshold_), goals.size() - 1);
    closest_node_ = std::min(goal_node_, start + static_cast<int>(subgoal_offset));
    ...
    reached_publisher_->publish(reached);
    if (!reached_goal_) {
      waypoint_publisher_->publish(waypoint_pose(prediction, subgoal_offset, waypoint_index_, stamp));
    }
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 5000,
      "node=%d/%d distance=%.2f inference=%.3fs",
      closest_node_, goal_node_, minimum_distance, duration.data);
  }
  ...
  double context_period_seconds_ = 0.25;
  double image_timeout_seconds_ = 2.0;
  double close_threshold_ = 3.0;
  double max_v_ = 0.2;
  double model_frame_rate_ = 4.0;
  int radius_ = 2;
  int waypoint_index_ = 2;
  int goal_node_ = 0;
  int closest_node_ = 0;
